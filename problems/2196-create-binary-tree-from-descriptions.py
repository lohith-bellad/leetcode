...
class Solution:
    def createBinaryTree(self, descriptions: List[List[int]]) -> Optional[TreeNode]:
        ...
        for parent, child, is_left in descriptions:
            if parent not in node_table:
                node_table[parent] = TreeNode(parent)

            child_set.add(child)
            if child not in node_table:
                node_table[child] = TreeNode(child)

            if is_left:
                node_table[parent].left = node_table[child]
            else:
                node_table[parent].right = node_table[child]
