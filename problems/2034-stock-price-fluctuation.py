...
class StockPrice:
    ...
    def update(self, timestamp: int, price: int) -> None:
        ...
        self.latest_stamp = max(self.latest_stamp, timestamp)

        heapq.heappush(self.min_heap, (price, timestamp))
        heapq.heappush(self.max_heap, (-price, timestamp))

    def current(self) -> int:
        return self.stocks[self.latest_stamp]

    def maximum(self) -> int:
        max_price, max_timestamp = self.max_heap[0]

        # old price has been updated on the timestamp
        while self.stocks[max_timestamp] != max_price:
    ...
    def minimum(self) -> int:
        min_price, min_timestamp = self.min_heap[0]

        # old price has been updated on the timestamp
        while self.stocks[min_timestamp] != min_price:
