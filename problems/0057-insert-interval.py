...
endi] represent the start and the end of the ith interval and intervals is sorted in
ascending order by starti. You are also given an interval newInterval = [start, end] that
...
class Solution:
    def insert(
        ...
        while ind < len(intervals) and intervals[ind][1] < new_x:
            output.append(intervals[ind])
            ind += 1

        while ind < len(intervals) and new_y >= intervals[ind][0]:
            new_x = min(new_x, intervals[ind][0])
            new_y = max(new_y, intervals[ind][1])
            ind += 1
