...
Implement the RandomizedCollection class:
    ...
    * bool remove(int val) Removes an item val from the multiset if present.
      Returns true if the item is present, false otherwise. Note that if val
...
class RandomizedCollection:
    ...
    def insert(self, val: int) -> bool:
        ...
        idx = len(self.list) - 1
        if val in self.element_table:
            self.element_table[val].add(idx)
            return False
        self.element_table[val] = set([idx])
        return True

    def remove(self, val: int) -> bool:
        if val not in self.element_table:
            return False

        idx = self.element_table[val].pop()
        last_elem = self.list[-1]

        if idx != len(self.list) - 1:
            self.element_table[last_elem].remove(len(self.list) - 1)
            self.element_table[last_elem].add(idx)
            self.list[idx] = last_elem

        self.list.pop()

        if not self.element_table[val]:
            del self.element_table[val]
