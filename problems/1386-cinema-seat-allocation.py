...
Example 1:
    ...
    groups, where seats marked in blue are already reserved and contiguous
    seats marked in orange are for one group.
...
class Solution:
    def maxNumberOfFamilies(self, n: int, reservedSeats: List[List[int]]) -> int:
        ...
        for row, cur_seats in seats.items():
            left = not (cur_seats & left_side)
            mid = not (cur_seats & middle)
            right = not (cur_seats & right_side)
