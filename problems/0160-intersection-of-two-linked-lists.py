...
Example 1:
    ...
    [5,6,1,8,4,5]. There are 2 nodes before the intersected node in A; There are 3 nodes
    before the intersected node in B.
    - Note that the intersected node's value is not 1 because the nodes with value 1 in
    A and B (2nd node in A and 3rd node in B) are different node references. In other
    words, they point to two different locations in memory, while the nodes with value 8
...
Example 2:
    ...
    There are 3 nodes before the intersected node in A; There is 1 node before the
...
Example 3:
    ...
    [1,5]. Since the two lists do not intersect, intersectVal must be 0, while skipA and
...
class Solution:
    def getIntersectionNode(
        ...
        if head1 and head2 and head1 == head2:
            return head1
