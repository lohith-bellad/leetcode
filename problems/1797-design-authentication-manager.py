...
Example 1:
    ...
    Explanation:
        ...
        authenticationManager.countUnexpiredTokens(6); // The token with tokenId "aaa"
                                                       // is the only unexpired one
                                                       // at time 6, so return 1.
        ...
        authenticationManager.renew("aaa", 8); // The token with tokenId "aaa" expired
                                               // at time 7, and 8 >= 7, so at time 8
                                               // the renew request is ignored.
        authenticationManager.renew("bbb", 10); // The token with tokenId "bbb" is
                                                // unexpired at time 10, so the renew
                                                // request is fulfilled and now the
                                                // token will expire at time 15.
        authenticationManager.countUnexpiredTokens(15); // The token with tokenId "bbb"
                                                        // expires at time 15, and the
                                                        // token with tokenId "aaa"
                                                        // expired at time 7, so
                                                        // currently no token is
...
class AuthenticationManager:
    ...
    def generate(self, tokenId: str, currentTime: int) -> None:
        if tokenId not in self.sessions:
            self.sessions[tokenId] = currentTime + self.ttl
        return

    def renew(self, tokenId: str, currentTime: int) -> None:
        if tokenId in self.sessions:
            if self.sessions[tokenId] > currentTime:
                self.sessions[tokenId] = currentTime + self.ttl
        return
    ...
    def countUnexpiredTokens(self, currentTime: int) -> int:
        ...
        for _, value in self.sessions.items():
