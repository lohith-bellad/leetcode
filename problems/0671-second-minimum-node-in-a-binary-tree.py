...
non-negative value, where each node in this tree has exactly either two or
zero sub-node. If the node has two sub-nodes, then this node's value is the
...
class Solution:
    def findSecondMinimumValue(self, root: Optional[TreeNode]) -> int:
        def traverse(root: TreeNode):
            ...
            if self.min < root.val < self.second_min:
