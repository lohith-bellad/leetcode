...
class LFUCache:
    ...
    def get(self, key: int) -> int:
        ...
        cur_key_count = node.counter
        cur_key_list = self.count2list[cur_key_count]
        cur_key_list.remove(node)
        if self.min_count == cur_key_count and cur_key_list.size == 0:
            self.min_count += 1
        cur_key_count += 1
        node.counter = cur_key_count

        if cur_key_count not in self.count2list:
            self.count2list[cur_key_count] = DLinkedList()
        self.count2list[cur_key_count].insert(node)
        return node.val
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if key in self.key2node:
            ...
            if self.capacity == self.size:
                min_list = self.count2list[self.min_count]
                d_node = min_list.remove()
                self.size -= 1
                del self.key2node[d_node.key]
            node = Node(key, value)
            self.key2node[key] = node
            if 1 not in self.count2list:
                self.count2list[1] = DLinkedList()
            self.count2list[1].insert(node)
            self.size += 1
            self.min_count = 1
