...
class Solution:
    def depthSumInverse(self, nestedList: List[NestedInteger]) -> int:
        ...
        for elem in nestedList:
            if elem.isInteger():
                standby.append((depth, elem.getInteger()))
            else:
                queue.append((depth, elem))

        max_depth = 1

        while queue:
            cur_depth, cur_elem = queue.popleft()

            if cur_elem.isInteger():
                standby.append((cur_depth, cur_elem.getInteger()))
            else:
                elems = cur_elem.getList()
                max_depth = max(max_depth, cur_depth + 1)
                for elem in elems:
                    queue.append((cur_depth + 1, elem))

        output = 0
        for depth, elem in standby:
            output += (max_depth - depth + 1) * elem
