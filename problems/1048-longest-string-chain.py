...
Example 3:
    ...
    Explanation: The trivial word chain ["abcd"] is one of the longest word
        chains. ["abcd","dbqca"] is not a valid word chain because the ordering
    ...
    def longestStrChain(self, words: List[str]) -> int:
        ...
        def dfs(cur_word):
            ...
            for i in range(len(cur_word)):
                search_word = cur_word[:i] + cur_word[i + 1 :]
                if search_word in words_set:
                    max_len = max(max_len, dfs(search_word) + 1)
